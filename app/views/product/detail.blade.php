...
<body>
  ...
  <main class="container detail-container">
            ...
            <div class="product-info">
                ...
                <div class="product-meta">
                    ...
                    <div class="meta-item">
                        ...
                        <span class="meta-value badge bg-primary bg-opacity-10 text-primary">{{ $product['size_name'] }}</span>
                    </div>
                    @endif
                    @if(!empty($product['color_name']))
                    <div class="meta-item">
                        <span class="meta-label">Màu sắc:</span>
                        <span class="meta-value badge bg-primary bg-opacity-10 text-primary">{{ $product['color_name'] }}</span>
                    </div>
                    @endif
                    <div class="meta-item">
                        <span class="meta-label">Tình trạng:</span>
                        <span class="meta-value text-success">Còn hàng ({{ $product['quantity'] }})</span>
                    </div>
                </div>

                <div class="product-description">
                    <h5 class="text-dark fw-bold mb-3">Mô tả sản phẩm</h5>
                    <p>{{ $product['description'] ? $product['description'] : 'Sản phẩm đang được cập nhật mô tả chi tiết. Vui lòng liên hệ hỗ trợ để biết thêm thông tin.' }}</p>
                </div>
                ...
                <div class="d-flex align-items-center gap-3 mt-5">
                    ...
                    <a href="/cart/add/{{ $product['id'] }}" class="btn btn-primary btn-buy flex-grow-1">Mua ngay</a>
                    <a href="/wishlist/toggle/{{ $product['id'] }}" 
                       class="btn btn-outline-danger px-3 py-3 rounded-3 btn-wishlist {{ $isInWishlist ? 'active' : '' }}" 
                       onclick="toggleWishlist(event, this)"
                       title="{{ $isInWishlist ? 'Xóa khỏi yêu thích' : 'Thêm vào yêu thích' }}">
                        <i class="{{ $isInWishlist ? 'fas' : 'far' }} fa-heart fa-lg"></i>
                    </a>
                    <a href="/cart/add/{{ $product['id'] }}" class="btn btn-outline-primary btn-add-cart">
    ...
    <div class="mt-5 pt-5 border-top">
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            <a href="/home/shop?category={{ $product['category_id'] }}" class="text-primary text-decoration-none fw-600">
        ...
        <div class="row g-4">
            ...
            <div class="col-6 col-md-3">
                <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative" style="transition: transform 0.3s ease;">
                    @if(!empty($item['image']))
                        <img src="/uploads/{{ $item['image'] }}" class="card-img-top" alt="{{ $item['name'] }}" style="height: 180px; object-fit: cover;">
                    @else
                    ...
                    <div class="card-body p-3">
                        <span class="text-primary fw-700 small text-uppercase" style="font-size: 0.65rem; letter-spacing: 0.5px;">{{ $item['category_name'] }}</span>
                        <h6 class="card-title text-truncate mb-2 mt-1" style="font-size: 0.95rem;">{{ $item['name'] }}</h6>
                        <div class="d-flex align-items-center justify-content-between">
                            <span class="text-primary fw-bold">{{ number_format($item['price']) }} đ</span>
                            <a href="/product/detail/{{ $item['id'] }}" class="btn btn-sm btn-link p-0 text-decoration-none text-dark fw-600" style="font-size: 0.8rem;">Chi tiết</a>
                        </div>
                    </div>
                </div>
            </div>
  ...
  <script>
    function updateQty(val) {
        ...
        if (current + val >= 1 && current + val <= {{ $product['quantity'] }}) {
